...
struct jpeg_compress_struct {
  ...
  bool raw_data_in;		/* TRUE=caller supplies downsampled data */
  ...
  bool write_Adobe_marker;	/* should an Adobe marker be written? */
...
// Modified
EXTERN(void) jpeg_stdio_dest JPP((j_compress_ptr cinfo, Uint16 inBytesWriteOnce));  
EXTERN(void) jpeg_stdio_src JPP((j_decompress_ptr cinfo, Uint16 inBytesReadOnce, Uint32 inBufferSize));       
...
EXTERN(bool) jpeg_start_decompress JPP((j_decompress_ptr cinfo));
EXTERN(JDIMENSION) jpeg_read_scanlines JPP((j_decompress_ptr cinfo,
					    JSAMPARRAY scanlines,
					    JDIMENSION max_lines));
EXTERN(bool) jpeg_finish_decompress JPP((j_decompress_ptr cinfo));
...
EXTERN(bool) jpeg_has_multiple_scans JPP((j_decompress_ptr cinfo));
EXTERN(bool) jpeg_start_output JPP((j_decompress_ptr cinfo,
				       int scan_number));
EXTERN(bool) jpeg_finish_output JPP((j_decompress_ptr cinfo));
EXTERN(bool) jpeg_input_complete JPP((j_decompress_ptr cinfo));
...
EXTERN(bool) jpeg_resync_to_restart JPP((j_decompress_ptr cinfo,
					    int desired));
...
typedef struct {
  ...
  const void* pSourceBuffer;	// buffer source for jpeg image
  Uint32 nBufferSize;			// buffer size 
  Uint32 nOffset;				// read offset 
  Uint32 nBytesInBuffer;		// number of bytes in the buffer 
  Uint16 nBytesReadOnce;		// number of bytes read by the library from pBuffer

  JOCTET * buffer;				/* start of buffer */
  bool bStart;					/* have we gotten any data yet? */
} my_source_mgr;
...
typedef struct {
  ...
  void* outBuffer;        // buffer destination for jpeg image
  Uint32 nBufferSize;     // buffer size (outBuffer)
  Uint32 nOffset;         // write offset (internal use)
  Uint16 nBytesWriteOnce; // number of bytes write by the library in outFile or outBuffer 
